...
def solution_model():
    ...
    # DO NOT CHANGE THIS CODE
    gpus = tf.config.experimental.list_physical_devices('GPU') # Extre
    tf.config.experimental.set_memory_growth(gpus[0], enable=True) # Extre
    tf.keras.backend.clear_session()
    print("Num GPUs:", len(tf.config.list_physical_devices('GPU'))) # Extre
    ...
    class myCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if (logs.get('mae') < 0.05):
                print("\nReached 0.05 or less MAE score so cancelling training!")
    ...
    model = tf.keras.models.Sequential([
        ...
        # YOUR CODE
        tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64, return_sequences=True, input_shape=[N_PAST, N_FEATURES])),
        tf.keras.layers.Dense(7, activation='sigmoid'),
        tf.keras.layers.Dense(N_FEATURES),
        tf.keras.layers.Reshape([N_FUTURE, -1])

        #tf.keras.layers.Conv1D(filters=32, kernel_size=5, strides=1, padding='causal', activation='relu',input_shape=[None, N_PAST, N_FEATURES]),
